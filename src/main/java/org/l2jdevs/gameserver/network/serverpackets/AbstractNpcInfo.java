...
import org.l2jdevs.gameserver.instancemanager.TownManager;
...
public abstract class AbstractNpcInfo extends L2GameServerPacket {
    ...
    private static final int NAME_COLOR_RED = 0xFF0000,
            NAME_COLOR_GOLD = 0xFFFF00, // yellow
            NAME_COLOR_DEFAULT = 0x0; // (green)
    ...
    protected int _titleColor = NAME_COLOR_DEFAULT; // client default
    ...
    public static class NpcInfo extends AbstractNpcInfo {
        ...
        public NpcInfo(L2Npc cha, L2Character attacker) {
            ...
            if (_npc.isInvisible()) {
                ...
                getChampionNameAndColor(cha); // affects: _title _titleColor
            ...
            if (Config.SHOW_NPC_LVL && (_npc instanceof L2MonsterInstance)) {
                if (_title != null && _title.length() > 0)
                    _title = cha.getLevel() + " " + _title;
                else
                    _title = String.valueOf(cha.getLevel());
				/*
                                if (cha.isAggressive())
                                    _titleColor = _titleColor == NAME_COLOR_DEFAULT
                                        ? NAME_COLOR_CRIMSON
                                        : NAME_COLOR_RED;
					*/
            }
        ...
        private void getChampionNameAndColor(final L2Npc cha) {
            /**
             * affects: this._title this._titleColor
             * Getting from L2Npc instance titles, names, colors, etc.
             * Also, see L2Attackable for champions and other unical things.
             * This is a packet generator, so it is reasonable to use only
             * static or already defined data.
             * (or else different users will see mob under different names).
             * L2JMOD_CHAMP_TITLE fallback title,
             * constant string for all and every champion
             * L2JMOD_CHAMPION_RANDNAMES if true poll mob for its title (name) and color
             * TODO: name coloring other from 0x0 (client default)
             */
            if (!cha.isChampion())
                return;
            _title = Config.L2JMOD_CHAMPION_RANDNAMES
                    ? cha.getChampionName()
                    : Config.L2JMOD_CHAMP_TITLE; // On every subclass
            if (Config.L2JMOD_CHAMPION_COLORED_NAMES)
                _titleColor = cha.isAggressive()
                        ? Config.L2JMOD_CHAMPION_NAME_COLOR_AGRESSIVE
                        : Config.L2JMOD_CHAMPION_NAME_COLOR_PEACEFUL;
            else
                _titleColor = Config.L2JMOD_CHAMPION_NAME_COLOR_DEFAULT;
        }
        ...
        protected void writeImpl() {
            ...
            int team = _npc.getTeam().getId();
            if (Config.L2JMOD_CHAMPION_ENABLE && team < 1)
                /* Champion's aura and name:
                 * power < 1/2 : name && no aura
                 * power in [1/2, 3/4] : blue aura && name
                 * power > 3/4 : red aura && name
                 * ?? 0 - 48 - 96 (%128)
                 */
                if (Config.L2JMOD_CHAMPION_ENABLE_AURA > 0 && _npc.isChampion()) {
                    final int ke4 = Config.L2JMOD_CHAMPION_HP / 4,
                        ke2 = Config.L2JMOD_CHAMPION_HP / 2;
                    if ((Config.L2JMOD_CHAMPION_HP - _npc.getMultiplierHP()) < ke4)
                        team = 2; // red
                    else
                        team = _npc.getMultiplierHP() > ke2 ? 1 : 0; // blue or none
                }
                else
                    team = 0;
            writeC(team);

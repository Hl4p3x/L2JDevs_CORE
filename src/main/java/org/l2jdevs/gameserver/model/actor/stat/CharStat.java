...
import org.l2jdevs.gameserver.model.stats.MoveType;
import org.l2jdevs.gameserver.model.stats.Stats;
import org.l2jdevs.gameserver.model.stats.TraitType;
...
public class CharStat
{
	...
	{
                double atk = _activeChar.getTemplate().getBaseMAtk();
		if (Config.L2JMOD_CHAMPION_ENABLE && _activeChar.isChampion())
                    atk *= _activeChar.getAtkBonusMultiplier();
                else if (Config.L2JMOD_NPC_POWER_JITTER_ENABLE)
                    atk *= _activeChar.getAtkBonusMultiplier();
		if (_activeChar.isRaid())
                    atk *= Config.RAID_MATTACK_MULTIPLIER;
		
		// Calculate modifiers Magic Attack
		return calcStat(Stats.MAGIC_ATTACK, atk, target, skill);
	}
	...
	{
                double spd = _activeChar.getTemplate().getBaseMAtkSpd();
		if (Config.L2JMOD_CHAMPION_ENABLE && _activeChar.isChampion())
                    spd *= _activeChar.getSpdBonusMultiplier();
                else if (Config.L2JMOD_NPC_POWER_JITTER_ENABLE)
                    spd *= _activeChar.getSpdBonusMultiplier();
		
		double val = calcStat(Stats.MAGIC_ATTACK_SPEED, spd);
	...
	{
                double atk = _activeChar.getTemplate().getBasePAtk();
		if (Config.L2JMOD_CHAMPION_ENABLE && _activeChar.isChampion())
                    atk *= _activeChar.getAtkBonusMultiplier();
                else if (Config.L2JMOD_NPC_POWER_JITTER_ENABLE)
                    atk *= _activeChar.getAtkBonusMultiplier();
		if (_activeChar.isRaid())
                    atk *= Config.RAID_PATTACK_MULTIPLIER;
		return calcStat(Stats.POWER_ATTACK, atk, target, null);
	}
	...
	{
		double spd = _activeChar.getTemplate().getBasePAtkSpd();
		if (Config.L2JMOD_CHAMPION_ENABLE && _activeChar.isChampion())
                    spd *= _activeChar.getSpdBonusMultiplier();
                else if (Config.L2JMOD_NPC_POWER_JITTER_ENABLE)
                    spd *= _activeChar.getSpdBonusMultiplier();
		return Math.round(calcStat(Stats.POWER_ATTACK_SPEED, spd, null, null));
	}

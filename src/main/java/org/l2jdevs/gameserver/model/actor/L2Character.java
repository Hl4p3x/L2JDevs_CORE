...
import static org.l2jdevs.gameserver.ai.CtrlIntention.AI_INTENTION_ACTIVE;
import static org.l2jdevs.gameserver.ai.CtrlIntention.AI_INTENTION_ATTACK;
...
import org.l2jdevs.gameserver.model.TimeStamp;
...
import org.l2jdevs.gameserver.model.actor.instance.L2RiftInvaderInstance;
import org.l2jdevs.gameserver.model.actor.knownlist.CharKnownList;
import org.l2jdevs.gameserver.model.actor.stat.CharStat;
import org.l2jdevs.gameserver.model.actor.status.CharStatus;
import org.l2jdevs.gameserver.model.actor.tasks.character.FlyToLocationTask;
import org.l2jdevs.gameserver.model.actor.tasks.character.HitTask;
import org.l2jdevs.gameserver.model.actor.tasks.character.MagicUseTask;
import org.l2jdevs.gameserver.model.actor.tasks.character.NotifyAITask;
import org.l2jdevs.gameserver.model.actor.tasks.character.QueuedMagicUseTask;
...
import org.l2jdevs.gameserver.model.actor.transform.TransformTemplate;
...
import org.l2jdevs.gameserver.model.holders.InvulSkillHolder;
...
import org.l2jdevs.gameserver.model.interfaces.ISkillsHolder;
...
import org.l2jdevs.gameserver.model.options.OptionsSkillHolder;
import org.l2jdevs.gameserver.model.options.OptionsSkillType;
...
import org.l2jdevs.gameserver.model.skills.SkillChannelized;
import org.l2jdevs.gameserver.model.skills.SkillChannelizer;
import org.l2jdevs.gameserver.model.skills.targets.L2TargetType;
import org.l2jdevs.gameserver.model.stats.Calculator;
...
import org.l2jdevs.gameserver.network.serverpackets.AbstractNpcInfo;
import org.l2jdevs.gameserver.network.serverpackets.ActionFailed;
import org.l2jdevs.gameserver.network.serverpackets.Attack;
import org.l2jdevs.gameserver.network.serverpackets.ChangeMoveType;
...
import org.l2jdevs.gameserver.network.serverpackets.ExRotation;
...
import org.l2jdevs.gameserver.network.serverpackets.MagicSkillCanceld;
import org.l2jdevs.gameserver.network.serverpackets.MagicSkillLaunched;
import org.l2jdevs.gameserver.network.serverpackets.MagicSkillUse;
import org.l2jdevs.gameserver.network.serverpackets.MoveToLocation;
import org.l2jdevs.gameserver.network.serverpackets.Revive;
import org.l2jdevs.gameserver.network.serverpackets.ServerObjectInfo;
...
import org.l2jdevs.gameserver.network.serverpackets.TeleportToLocation;
import org.l2jdevs.gameserver.pathfinding.AbstractNodeLoc;
...
import org.l2jdevs.util.EmptyQueue;
...
public abstract class L2Character extends L2Object implements ISkillsHolder, IDeletable
{
	private static final Logger LOG = LoggerFactory.getLogger(L2Character.class);
        private static final String NPC_DEFAULT_NAME = "";
	...
	public void onHitTimer(L2Character target, int damage, boolean crit, boolean miss, boolean soulshot, byte shld)
	{
		...
		// than 8 levels lower
		if (target.isRaid() && target.giveRaidCurse() && shouldbeRaidCursed(target))
		{
		...
		if (!miss && (damage > 0))
		{
			...
			if (!isBow && !target.isInvul()) // Do not reflect if weapon is of type bow or target is invunlerable
			{
                            ...
                            if (!target.isRaid() || (getActingPlayer() == null) || shouldbeRaidCursed(getActingPlayer(), target))
				{
			...
			for (L2Object obj : targets)
			{
				...
				if (((target.isRaid()
                                     ...
                                     || (!skill.isBad()
                                         ...
                                         && shouldbeRaidCursed(targetsAttackTarget))
                                     || (!skill.isBad()
                                         ...
                                         && shouldbeRaidCursed(targetsCastTarget))))
				{
	...
	public final int getShldDef()
	{
        ...
        /**
         * @return true iff PC's sex is female
         */
        private boolean isFemale() {return getActingPlayer().getAppearance().getSex();}

        public int getSTR()
	{
            int ri = getStat().getSTR();
            if(Config.L2JMOD_SEX_DIFF && isPlayer() && !isFemale())
                ri += 2;
            return ri;
	}
	...
	public void reduceCurrentHp(double i, L2Character attacker, boolean awake, boolean isDOT, Skill skill) {
            double dmg = i / getMultiplierHP();
            getStatus().reduceHp(dmg, attacker, awake, isDOT, false);
            // TODO: me.setAfraid if me.currentHP < 10% and me is L2Npc and not (me is undead or me is golem)
            //EventDispatcher.getInstance().notifyEventAsync(EventType.FEAR);
	}
	...
	public boolean isChampion()
	{
        ...
        public String getChampionName() {
            return NPC_DEFAULT_NAME; // nameless mobs by-default
        }
         ...
         * incoming damage _divided_ by this value
         */
        ...
        /**
         * used for P/M attack power related calculations
         * ex: bonusAtk = _activeChar.getAtkBonusMultiplier();
         */
        public double getAtkBonusMultiplier() {
            return 1;
        }

        public double getAtkBonusMultiplier(double n) {
            return n;
        }
	...
	{
		return !Config.RAID_DISABLE_CURSE;
	}
    ...
    /**
     * Check Raidboss attack
     * Character should be petrified if attacking a raid that's more:
     * - than 8 levels lower
     * + than Config.L2JMOD_RAID_CURSE_LEVEL_DIFF levels lower
     */
    private boolean shouldbeRaidCursed(final L2Character target) {
        if (Config.RAID_DISABLE_CURSE)
            return false;
        return (getLevel() - target.getLevel()) > Config.L2JMOD_RAID_CURSE_LEVEL_DIFF;
    }

    private static boolean shouldbeRaidCursed(final L2PcInstance src, final L2Character target) {
        if (Config.RAID_DISABLE_CURSE)
            return false;
        return (src.getLevel() - target.getLevel()) > Config.L2JMOD_RAID_CURSE_LEVEL_DIFF;
    }

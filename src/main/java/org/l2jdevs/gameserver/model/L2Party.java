...
import org.l2jdevs.gameserver.model.entity.DimensionalRift;
...
import org.l2jdevs.gameserver.network.serverpackets.ExAskModifyPartyLooting;
import org.l2jdevs.gameserver.network.serverpackets.ExCloseMPCC;
import org.l2jdevs.gameserver.network.serverpackets.ExOpenMPCC;
import org.l2jdevs.gameserver.network.serverpackets.ExPartyPetWindowAdd;
import org.l2jdevs.gameserver.network.serverpackets.ExPartyPetWindowDelete;
import org.l2jdevs.gameserver.network.serverpackets.ExSetPartyLooting;
import org.l2jdevs.gameserver.network.serverpackets.L2GameServerPacket;
import org.l2jdevs.gameserver.network.serverpackets.PartyMemberPosition;
import org.l2jdevs.gameserver.network.serverpackets.PartySmallWindowAdd;
import org.l2jdevs.gameserver.network.serverpackets.PartySmallWindowAll;
import org.l2jdevs.gameserver.network.serverpackets.PartySmallWindowDelete;
import org.l2jdevs.gameserver.network.serverpackets.PartySmallWindowDeleteAll;
...
public class L2Party extends AbstractPlayerGroup
{
	...
	private long _pendingInviteTimeout;
        private int _partyLvl = 0, avgLvl = 0, minLvl = 0; // also used by AI for attack decision
	 ...
	 * @see org.l2jdevs.gameserver.model.actor.instance.L2PcInstance#isRequestExpired()
	 */
	...
	public void addPartyMember(L2PcInstance player)
	{
		...
		if (player.getLevel() > _partyLvl)
		{
			_partyLvl = player.getLevel();
                ...
                if(minLvl > player.getLevel())
                    minLvl = player.getLevel();
			...
			if (member != null)
			{
				...
				if (summon != null)
				{
					summon.updateEffectIcons();
                                ...
                                avgLvl += member.getLevel();
                                ++n;
			}
		}
                avgLvl /= n; // avoid rounding errors as in: avgLvl = (avgLvl + player.getLevel()) / 2;
		...
		for (L2PcInstance member : getMembers())
		{
			...
			if (member.getLevel() > newLevel)
			{
				newLevel = member.getLevel();
                        ...
                        n++;
                        avgLvl += member.getLevel();
                        minLvl = Integer.min(minLvl, member.getLevel());
		}
		_partyLvl = newLevel;
                avgLvl /= n;
	}

...
import org.l2jdevs.gameserver.instancemanager.TerritoryWarManager.Territory;
...
import org.l2jdevs.gameserver.model.itemcontainer.ClanWarehouse;
...
import org.l2jdevs.gameserver.network.serverpackets.ExSubPledgeSkillAdd;
...
import org.l2jdevs.gameserver.network.serverpackets.PledgeReceiveSubPledgeCreated;
import org.l2jdevs.gameserver.network.serverpackets.PledgeShowInfoUpdate;
import org.l2jdevs.gameserver.network.serverpackets.PledgeShowMemberListAll;
import org.l2jdevs.gameserver.network.serverpackets.PledgeShowMemberListDeleteAll;
...
import org.l2jdevs.gameserver.network.serverpackets.PledgeSkillList.SubPledgeSkill;
import org.l2jdevs.gameserver.network.serverpackets.PledgeSkillListAdd;
...
public class L2Clan implements IIdentifiable, INamable
{
	...
	private void restoreRankPrivs()
	{
			...
			PreparedStatement ps = con.prepareStatement("SELECT `rank`, `party`, `privs` FROM `clan_privs` WHERE `clan_id` = ?"))
		{
			...
			if (Config.DEBUG)
			{
				_log.warning("clanPrivs restore for ClanId: " + getId());
			}
			...
			try (ResultSet rs = ps.executeQuery())
			{
				...
				while (rs.next())
				{
					...
					final int party = rs.getInt("party");

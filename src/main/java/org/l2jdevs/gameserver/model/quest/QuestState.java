...
import org.l2jdevs.gameserver.network.serverpackets.ExShowQuestMark;
import org.l2jdevs.gameserver.network.serverpackets.QuestList;
import org.l2jdevs.gameserver.network.serverpackets.ShowMiniMap;
import org.l2jdevs.gameserver.network.serverpackets.TutorialCloseHtml;
import org.l2jdevs.gameserver.network.serverpackets.TutorialEnableClientEvent;
...
public final class QuestState
{
	 ...
	 * @see org.l2jdevs.gameserver.model.quest.State
	 */
	...
	{
            giveItems(Inventory.ADENA_ID, applyRates ? (long)(count * Config.RATE_QUEST_REWARD_ADENA) : count);
	}
	 ...
	 * @param applyRates If {@code true}, multiply the amount by the configured Rate Quest Reward
	 */
	public void giveAdenaFuzzy(final long count, final boolean applyRates) {
            long cash = count / 2 + AbstractScript.getRandom(count);
            if(applyRates)
                cash *= Config.RATE_QUEST_REWARD_ADENA;
            giveItems(Inventory.ADENA_ID, cash);
	}
	 ...
	 * @param applyRates If {@code true}, multiply the amount by the configured Rate Quest Reward
	 */
	public void giveAdenaFuzzyHalf(final long count, final boolean applyRates) {
            long cash = count / 2 + AbstractScript.getRandom(count / 2);
            if(applyRates)
                cash *= Config.RATE_QUEST_REWARD_ADENA;
            giveItems(Inventory.ADENA_ID, cash);
	}
	...
	public void giveItems(int itemId, long count)
	{
            if (Config.L2JMOD_ALLOW_QUEST_SSHOTS
                || itemId != SOULSHOT_NO_GRADE_FOR_BEGINNERS && itemId != SPIRITSHOT_NO_GRADE_FOR_BEGINNERS)
    ...
    public String toString() {
        return String.format("name '%s'  state %d  pcId %d",
                             _questName, _state, _player.getId());
    }
}
